...
class profiles::testing::jenkins (
  ...
  Hash $plugins = {},
  Hash $plugins_casc = {
    command-launcher => { version => '1.4'},
    configuration-as-code => {version => '1.35'},
    credentials => {},
    jaxb => { version => '2.3.0.1'},
    jdk-tool => { version => '1.4'},
    script-security => { version => '1.69'},
    ssh-credentials => {},
    structs => {},
    trilead-api => { version => '1.0.5'},
  },
